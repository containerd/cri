...
const (
	...
	// targetImageDigestLabel is a label which contains image digest and will be passed
	// to snapshotters.
	targetImageDigestLabel = "containerd.io/snapshot/cri.image-digest"
			...
			case imagespec.MediaTypeImageManifest, containerdimages.MediaTypeDockerSchema2Manifest:
				for i := range children {
					...
					if containerdimages.IsLayerType(c.MediaType) {
						...
						c.Annotations[targetImageDigestLabel] = desc.Digest.String()
					}
				}
			}
			return children, nil
